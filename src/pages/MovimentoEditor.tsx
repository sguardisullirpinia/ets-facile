...
function tokenizeSemanticText(text: string) {
  return applyCommonFixes(text).split(" ").map((x) => x.trim()).filter(Boolean);
}
...
function getSemanticHint(primaryLabel: string, detailLabel: string) {
  const exactDetail = SEMANTIC_HINTS[applyCommonFixes(detailLabel)];
  ...
  for (const item of CONFIG_REGISTRY) {
    ...
    for (const primary of item.config.primary) {
      ...
      for (const rawDetail of details) {
        ...
        const detailLabel = detail || primary.label;
        const hint = getSemanticHint(primary.label, detailLabel);

        const alias = uniqueStrings([
          detailLabel,
          primary.label,
          categoriaLabel,
          `${primary.label} ${detailLabel}`,
          ...(hint.alias || []),
        ]);

        const triggerWords = uniqueStrings([
          ...tokenizeSemanticText(detailLabel),
          ...(hint.triggerWords || []),
        ]);
